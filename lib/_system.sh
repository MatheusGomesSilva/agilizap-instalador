...
system_mv_folder() {
  print_banner
  printf "${WHITE} 💻 Fazendo download do código AGILIZAP...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2

  sudo su - root <<EOF
  cp "${PROJECT_ROOT}"/agilizap.zip /home/deploy/${instancia_add}/
EOF
...
#   None
#######################################
system_unzip_agilizap() {
  print_banner
  printf "${WHITE} 💻 Fazendo unzip agilizap...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2

  sudo su - deploy <<EOF
  unzip /home/deploy/${instancia_add}/agilizap.zip -d /home/deploy/${instancia_add}
EOF
